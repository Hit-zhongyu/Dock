...
from .mmcif import get_data_bioassembly
...
def get_ligand_cluster_id(sample_indice):
    """提取 sample_indice 中 ligand 对应的 cluster_id"""
    if sample_indice["mol_1_type"] == "ligand":
        return sample_indice["cluster_1_id"]
    elif sample_indice["mol_2_type"] == "ligand":
        return sample_indice["cluster_2_id"]
    else:
        return None
...
class BaseSingleDataset(Dataset):
    ...
    def _get_bioassembly_data(
        ...
        bioassembly_dict = get_data_bioassembly(
    ...
    def process_one(
        ...
        ligand_cluster_id = get_ligand_cluster_id(sample_indice)
        ...
        feat, label, label_full = self.get_feature_and_label(
            ...
            is_spatial_crop=False,
        )

        feat['smile'] = bioassembly_dict['ligand_features'][ligand_cluster_id]['smile']
        ...
        asymn_id_to_chain_id = {
            atom.asym_id_int: atom.chain_id for atom in bioassembly_dict["atom_array"]
        }
        chain_id_list = [
...
def get_datasets(
    ...
    for train_name in data_config.train_sets:
        ...
        # train_datasets[train_name] = train_dataset
