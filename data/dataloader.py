...
class UniformSampler(Sampler):
    """
    A uniform random sampler for a single node.
    Samples data points uniformly at random (no weights).
    """
    ...
    def __init__(
        ...
        """
        Args:
            dataset_size (int): The size of the dataset to sample from.
            num_samples (int): The number of samples to be drawn.
...
class DistributedWeightedSampler(DistributedSampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        # ).tolist()
        indices = torch.randint(
            low=0, high=self.dataset_size, size=(self.num_samples,), generator=g,
        ).tolist()
